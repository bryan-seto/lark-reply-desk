...
**Goal:** Make every desk draft trustworthy enough that one "Follow Up All" button sends them — by fixing today's diff findings deterministically, then auto-learning honorifics/timing/style from what You actually sends and from unflag events.
...
2. **Capture + distill learning loop**: deterministic capture of (desk_draft vs user_actual_sent) pairs and unflag events into JSONL; a nightly distiller turns them into corrections (`flagged-desk-corrections.md`) and roster updates. Honorific learning is pure regex (deterministic); style/timing lessons go through the existing corrections→prompt wiring.
3. **Follow Up All button**: UI batch action gated by a deterministic trust-gate checklist per row, with a send-time freshness re-check in the daemon. You confirms once; daemon sends sequentially; results feed back into the capture loop.
...
- Desk used `ka @Contact C` / `ka @Contact D` — both are **pak**. (honorific gap)
- Desk always generated "circling back" nudge copy; You sent substantive content same-day (offers, specific questions like "July / Aug / Sept?", decisions like "Okie means 24th"). (intent gap)
- Desk timing throttled to ~2-working-day waits; You's rule is "if I have something to say, send now" — timers only for genuine waiting. (timing gap)
- Rows where `last_from: You` today were still shown "due now". (staleness gap)
- `pending_fix=True` rows where You sent a warm thanks (Contact E 谢谢) still need the commitment-date status nudge — thanks ≠ closure. Desk got this right; keep it.

---

## TWO-REPO COMMIT RULE (NEMESIS-proven)

Daemon/state files live in repo **A**: `/Users/user.seto/.hermes/profiles/user/` (verify: `git -C /Users/user.seto/.hermes/profiles/user rev-parse --show-toplevel`).
UI files live in repo **B**: `/Users/user.seto/lark-reply-desk/`.
...
```bash
cd /Users/user.seto/lark-reply-desk && /Users/user.seto/.hermes/profiles/user/home/.local/bin/node node_modules/.bin/next build
launchctl kickstart -k gui/$(id -u)/com.user.lark-reply-desk
...
**File:** `/Users/user.seto/.hermes/profiles/user/state/honorifics.json` (new)

```json
{
  "_comment": "Per-person honorific roster. source: seed|learned. Counts updated by honorific_learner.",
  "people": {
    "Contact C":          {"honorific": "pak", "source": "seed-2026-06-12", "counts": {"pak": 1}},
    "Contact D":          {"honorific": "pak", "source": "seed-2026-06-12", "counts": {"pak": 1}},
    ...
    "Contact E":                {"honorific": "",    "source": "seed-2026-06-12", "counts": {}},
    "Muhammad Fachri":       {"honorific": "pak", "source": "seed-2026-06-12", "counts": {}},
    "Contact A Yang":            {"honorific": "bu",  "source": "seed-2026-06-12", "counts": {}}
  },
  "banned": ["mas"],
  "default": ""
}
...
Seed values: pak for Contact C/Contact D confirmed from live sends; others from `user-reply-style` honorifics table + `stakeholder-roster` roster. **Recon step for implementer:** before writing, grep `user-reply-style` skill (`skills/domain/user-reply-style/`) for its honorifics table and copy every person it lists. Empty string = no honorific (plain name is correct).

**Verify:** `python3 -c "import json; json.load(open('/Users/user.seto/.hermes/profiles/user/state/honorifics.json'))"` → no error.
...
**New output:** `/Users/user.seto/.hermes/profiles/user/state/draft-vs-sent.jsonl`
...
def capture_draft_vs_sent(old_row: dict, fresh_thread: list[dict]) -> None:
    """Record (desk draft, what You actually sent) pairs for the learning loop."""
    ...
    rec = {
        ...
        "desk_draft": draft,
        "user_sent": [m["text"] for m in new_you],
        "user_sent_at": new_you[0]["t"],
        "distilled": False,
    }
...
Dedup guard: keep a set of `(handle, user_sent_at)` already captured (scan the file's last ~500 lines at startup) so re-harvests don't duplicate.
...
**File:** same harvester. **New output:** `/Users/user.seto/.hermes/profiles/user/state/unflag-events.jsonl`
...
Semantics this captures: an unflag where `last_from == You` ≈ "You considered his own last message the closer"; unflag where `pending_fix=True` ≈ "You closed despite pending fix" (timing lesson: he trusts the commitment). The distiller (Phase 2) interprets; capture stays dumb and deterministic.
...
**File:** `/Users/user.seto/.hermes/profiles/user/daemons/draft_validator.py` (new)
...
STATE = Path("/Users/user.seto/.hermes/profiles/user/state")
...
**Test first:** `daemons/tests/test_draft_validator.py` — cases: (a) `"hi ka @Contact C"` → fail with honorific reason; (b) `"hi pak @Contact C"` → pass; (c) em-dash gets scrubbed and passes; (d) `"(hermes should summarise context here)"` → fail; (e) `"Mas Andro"` → fail banned; (f) empty → fail. Run, see fail, implement, see pass.

### Task 1.2: Wire validator into draft_engine output path

**File:** `/Users/user.seto/.hermes/profiles/user/daemons/draft_engine.py`

In `draft_reply` and `draft_followup`, after `_extract_draft()` / `_has_placeholder` and before returning: run `draft_validator.validate(text)`. If `ok` → return `scrubbed` text. If only mechanical failures (em-dash/markdown) → return scrubbed. If honorific/placeholder failure → ONE retry through the existing fallback path with the failure reason appended to the prompt ("fix: roster says pak Contact C"); if still failing → return None (blank draft box beats wrong draft). Store the validation result so the harvester can persist it (next task).
...
```
b) **Staleness demotion (today's gap #4):** after the thread pull, if the latest thread message is from "You" AND its timestamp > `drafted_at` (or no draft yet) AND `followup_basis` in `("they_owe_reply", "waiting_no_commitment")` → the ball is in THEIR court because You just spoke. Force `suggested_date` to at least `last_you_ts + nudge_window(basis)` and set `next_action` to `"You replied {when} — waiting on {person}; nudge {date} if silent."` This kills the false "due now" rows.
c) **pending_fix + You-thanked rule (keep what worked):** when `pending_fix=True` and You's latest message is a pure ack/thanks (≤ 60 chars, matches `re.compile(r"thank|thanks|谢谢|辛苦|🙏|okie|noted", re.I)`), do NOT close; keep the commitment-date nudge and set `next_action` to `"Thanks sent. Fix still pending — confirm it ships by {date}."`

**Test:** `daemons/tests/test_staleness_demotion.py` — (a) You replied after draft → date pushed out; (b) other person replied last → unchanged; (c) pending_fix + 谢谢 → date stays at commitment date, action text says confirm-ships.

### Task 1.4: Encode today's stance lessons in the corrections file (reaches prompts via existing wiring)

**File:** `/Users/user.seto/.hermes/profiles/user/state/flagged-desk-corrections.md` — append:

```markdown
## 2026-06-12 — diff session lessons (desk draft vs You's live sends, 39 rows)
- INTENT: You does not send "circling back / keeping it on ur radar" filler when he has
...
- TIMING: if You has new info or a sharper question, the right time is NOW, not +2 working
  days. The 2-day window applies only when there is genuinely nothing new to add.
- HONORIFICS: Contact C = pak. Contact D = pak. Never "ka" for either.
...
### Task 1.5: Update `user-reply-style` skill honorifics table

Patch the skill's honorifics table (it feeds the hermes -z primary path) with: Contact C = pak, Contact D = pak. Use `skill_manage(action='patch')` on `domain/user-reply-style`. The engine reads skills live, so this self-propagates — that is the whole point of Path B.
...
Restart daemons: `launchctl kickstart -k gui/$(id -u)/com.user.lark-followup-harvest` and same for `com.user.lark-draft-pusher`. Verify next cycle log shows rows gaining `validation` fields:
```bash
tail -1 /Users/user.seto/.hermes/profiles/user/state/lark-followup-queue.jsonl | python3 -c "import json,sys; r=json.load(sys.stdin); print(r.get('validation'))"
```
...
**File:** `/Users/user.seto/.hermes/profiles/user/daemons/honorific_learner.py` (new)

Reads `draft-vs-sent.jsonl` records where `distilled=False`, extracts honorific usages from `user_sent` texts with the same `HONORIFIC_RE`, increments `counts` in `honorifics.json`, and flips `honorific` when a learned form reaches **3 observations AND a majority** (protects against one-off typos). Never overrides a `seed-*` entry downward to empty; logs every flip to stderr. ~40 lines. Called at the end of each harvester cycle (cheap) — no new daemon.

**Test first:** `daemons/tests/test_honorific_learner.py` — (a) 3× "pak @NewPerson" → roster gains NewPerson=pak; (b) 1× "ka @contact-c" does NOT flip seed pak; (c) counts accumulate across runs.

### Task 2.2: Deterministic timing tuner

**File:** `/Users/user.seto/.hermes/profiles/user/daemons/timing_tuner.py` (new)
**Output:** `/Users/user.seto/.hermes/profiles/user/state/timing-windows.json`

From `draft-vs-sent.jsonl` + `unflag-events.jsonl`, per `followup_basis`, compute the median delta between `suggested_date` and when You actually acted (`user_sent_at` / unflag `captured_at`). Write per-basis nudge windows:
...
Harvester's date suggestion reads this file (fallback to current hardcoded 2-working-days when `n < 5`). Today's evidence says You acts ~same-day when he has content — expect these windows to shrink where the data supports it.
...
Skill edits (e.g. new honorific table rows in `user-reply-style`) stay HUMAN-gated: distiller proposes in its summary; You approves; a follow-up session patches the skill. Rationale: skills feed every hermes -z draft — a bad auto-edit poisons everything silently.
...
When You clicks Unflag, show three one-tap reasons: `done` / `they'll handle it` / `not important`. POST includes `unflag_reason`; daemon writes it onto the matching `unflag-events.jsonl` record. One tap turns an ambiguous signal into labeled training data. Skippable (default `done` after 3s). Keep it ONE tap — anything heavier and You won't use it.
...
cd /Users/user.seto/lark-reply-desk
...
2. `followup_basis NOT IN ("closed",)` and `is_monitoring == false` (monitoring rows need You's own data check — never auto-send)
3. `validation.ok == true` (Phase 1 gate: honorifics, placeholders, ids, scrubbed)
4. `draft_text` non-empty
5. NOT stale: `last_from != "You"` OR `pending_fix == true` (a pending-fix confirm nudge is valid even after You's thanks)
...
Confirm → POST `/api/followup-all` with `[{handle, draft_text}]` (the possibly-edited texts). This is one human confirmation for the batch — matches You's existing approval model (review once, then it executes).
...
3. Else send via existing send path (lark-cli `code==0` = success), mark `sent`, and append a `draft-vs-sent.jsonl` record with `user_sent=[draft_text]`, `source="followup_all"` (auto-sends are training data too).
...
- **auto_due_today** (later, only after ≥2 weeks of batch results show ~0 bad sends): a morning cron runs gate+freshness and sends without the modal, then DMs You the summary in self-DM. DO NOT build the cron yet — just keep the gate logic callable headless (it already is: daemon-side). YAGNI on the cron until the trust data exists.
...
1. **Validator live:** queue rows carry `validation` objects; a deliberately bad draft (`ka @contact-c`) injected via test harness gets rejected/retried.
2. **Staleness:** rows where You replied today no longer show "due now"; desk "due now" count drops from 22 to the true waiting set.
3. **Capture flowing:** after You's next few replies in flagged threads, `draft-vs-sent.jsonl` grows; after next unflag, `unflag-events.jsonl` grows.
4. **Learning:** after ~a week, `honorifics.json` counts populate; distiller cron appends dated lessons; corrections stay ≤120 lines.
5. **Follow Up All:** with ≥3 due sendable rows, button shows correct N; modal exclusions list honest reasons; confirmed batch sends appear in Lark (You eyeballs 2–3); skipped-stale rows reported.
6. **Trust metric (the real goal):** weekly, % of desk drafts You sends unedited via the modal. When it's ≥80% for 2 weeks → discuss flipping `auto_due_today`.
...
- **Auto-send blast radius:** monitoring rows and You-replied-last rows are hard-excluded; freshness re-check is daemon-side at send time, not just UI-side at render time — drafts freeze at harvest, threads don't.
